...
def create_new_bot(telegram, theme, telegram_id, nickname):
    while True:
        ...
        while True:
            if res_bot_id == 3:
                create_username(telegram, str(uuid4())[:4] + str(uuid4())[:4], theme)
                res_bot_id = create_bot_id(telegram, telegram_id + str(uuid4())[:4], theme)  # пишем бот фазеру id бота
                # пишем бот фазеру название бота
                continue
            elif res_bot_id == 4:
                create_username(telegram, nickname + str(uuid4())[:4], theme)
                res_bot_id = create_bot_id(telegram, telegram_id + str(uuid4())[:4], theme)
                continue
            elif res_bot_id == 0:
...
def thread_function(telegram, theme, telegram_id, nickname):
    get_chats = telegram.get_chats()
    get_chats.wait()
    while True:
        # listen_rabit
...
def main(theme, telegram_id, nickname):
    for session in sessions:
        ...
        tg = Telegram(
            ...
            files_directory=f'./24e12581b67470754ba08957bb84de5c'
        )
        print("loging")
        tg.login()
        x = threading.Thread(target=telega, args=(tg,))
        x.start()
        res = thread_function(tg, theme, telegram_id, nickname)
...
print(main('dfgdfgfddf', "vsgugdfgcghsb", "jbsdfgdjkjs"))